...
def _composite_rebased_policy(
    state: dict,
    entry_mark: float,
    lots: int,
    *,
    source: str = "operator_pyramid_composite",
) -> dict:
...
def _pyramid_rebased_policy(state: dict, entry_mark: float, lots: int) -> dict:
    return _composite_rebased_policy(
        state, entry_mark, lots, source="operator_pyramid_composite",
    )


def _average_rebased_policy(state: dict, entry_mark: float, lots: int) -> dict:
    return _composite_rebased_policy(
        state, entry_mark, lots, source="operator_average_composite",
    )
...
def _average_base_state(state: dict, *, dry_run: bool) -> dict:
    if str(state.get("status") or "").upper() != "OPEN":
        raise RuntimeError("There is no open Trend position to average")
    if bool(state.get("dry_run")) != bool(dry_run):
        raise RuntimeError("The open position belongs to a different trading mode")
    if str(state.get("ownership") or "") not in {
        TREND_SCORE_AUTO_OWNERSHIP,
        ...
        raise RuntimeError(
            "External or ownership-ambiguous positions cannot be averaged"
        )
...
def _average_preview_from_state(
    state: dict,
    *,
    dry_run: bool,
    credentials: tuple[str, str] | None = None,
    ...
    if dry_run:
        ...
        executable = _trend_score_auto_number(
            quote.get("bid") if sign < 0 else quote.get("ask"),
            "average executable price", positive=True,
        )
        current_gross = (
            (mark - live_entry) * base["contract_value"] * lots * sign
        )
        ...
        build_trend_score_live_market_payload(
            prepared,
            quote,
            client_order_id="nithi-avg-preview",
            max_slippage_pct=limits[0],
            max_spread_pct=limits[1],
            max_quote_age_sec=limits[2],
        )
    ...
    if not math.isfinite(current_pnl) or current_pnl >= 0:
        raise RuntimeError(
            "Averaging is allowed only while the current P&L is negative"
        )
...
def _average_clear_intent(state: dict, *, note: str) -> dict:
    latest = _trend_score_auto_strict_json(_slot_file("trend"), {})
    if (
        str(latest.get("position_cycle_id") or "")
        != str(state.get("position_cycle_id") or "")
    ):
        raise RuntimeError("Trend position changed during average reconciliation")
    latest.update({
        "pending_average_intent": None,
        "last_average_reconciliation_note": note,
        "last_average_reconciliation_utc": datetime.now(
            timezone.utc
        ).isoformat(),
    })
    _atomic_write_json(_slot_file("trend"), latest)
...
def _average_rollback_partial(
    state: dict,
    *,
    filled_lots: int,
    credentials: tuple[str, str],
) -> dict:
    """Remove only a partially filled average add-on and prove the base is restored."""
    if filled_lots <= 0:
        return _average_clear_intent(state, note="average_not_filled")
    product_id = int(state["product_id"])
    client_id = (
        f"nithi-avg-rb-{int(time.time())}-{secrets.token_hex(4)}"
    )[:32]
    close_side = "buy" if state.get("side") == "short" else "sell"
    payload = {
        "product_id": product_id,
    ...
    if not order:
        raise RuntimeError(
            "Partial average rollback was rejected: "
            + str(response.get("error") or response)[:300]
        )
    order = _validate_dashboard_order(
        order,
    ...
    if not conclusive or rolled_back != filled_lots:
        raise RuntimeError(
            "Partial average rollback could not be fully verified; "
            "the position remains locked for reconciliation"
        )
    restored = _strict_realtime_position(
        product_id, credentials=credentials,
    ...
    if int(float(restored.get("size") or 0)) != expected:
        raise RuntimeError(
            "Partial average rollback did not restore the original quantity"
        )
    latest = _average_clear_intent(
        state, note="partial_average_fully_rolled_back",
    )
    _trend_audit("trend_average_partial_reversed", {
        "product_id": product_id,
        "filled_lots": filled_lots,
        "rollback_order_id": (order or {}).get("id"),
        "restored_lots": abs(expected),
...
def _average_apply_dry_run(state: dict, preview: dict) -> dict:
    """Atomically turn one simulated position into its composite average."""
    latest = _trend_score_auto_strict_json(
        _slot_file("trend", dry_run=True), {},
    )
    if (
        ...
        raise RuntimeError("DRY RUN position changed before averaging")
    added_lots = int(preview["add_lots"])
    total_lots = int(preview["total_lots"])
    fill = float(preview["estimated_fill"])
    entry_fee = float(
    ...
    now = datetime.now(timezone.utc).isoformat()
    events = [
        dict(item) for item in (latest.get("average_events") or [])
        if isinstance(item, dict)
    ][-19:]
    events.append({
        "at_utc": now,
    ...
    latest.update({
        ...
        "average_count": int(latest.get("average_count") or 0) + 1,
        "average_added_lots": int(
            latest.get("average_added_lots") or 0
        ) + added_lots,
    ...
    _atomic_write_json(_slot_file("trend", dry_run=True), latest)
    _trend_audit("trend_position_averaged", {
        "mode": "dry_run",
        "product_id": latest.get("product_id"),
        "previous_lots": preview["current_lots"],
        "added_lots": added_lots,
...
@app.route("/api/average/preview", methods=["POST"])
def api_average_preview():
    body = request.get_json(silent=True) or {}
    try:
        dry_run, mode = _pyramid_target_mode(body)
        state = _trend_score_auto_strict_json(
        ...
        credentials = None if dry_run else _active_creds()
        preview = _average_preview_from_state(
            state, dry_run=dry_run, credentials=credentials,
        )
        return jsonify({"ok": True, "target_mode": mode, **preview})
    except Exception as exc:
...
@app.route("/api/average/execute", methods=["POST"])
def api_average_execute():
    """Add one equal-size lot to the active Trend position to average down, once per cycle."""
    body = request.get_json(silent=True) or {}
    try:
        dry_run, mode = _pyramid_target_mode(body)
    except Exception as exc:
    ...
    user = _active_user()
    root_dir = _user_dir()
    owner = f"average:{user}:{os.getpid()}:{time.time_ns()}"
    try:
        with account_entry_lock(root_dir, owner) as exposure_lock:
            if not exposure_lock:
                raise RuntimeError(
            ...
            with account_file_lock(
                ...
                if dry_run:
                    updated = _average_apply_dry_run(state, preview)
                    return jsonify({
                        "ok": True,
                        "status": "OPEN",
                        "target_mode": mode,
                ...
                if int(refreshed_affordability.get("selected_lots") or 0) != int(
                    ...
                    raise RuntimeError(
                        "Available USD balance or executable depth changed; "
                        "the full matching average quantity is no longer safe"
                    )
                client_id = (
                    f"nithi-avg-{int(time.time())}-{secrets.token_hex(4)}"
                )[:32]
                limits = _trend_score_auto_live_execution_limits(prepared)
                payload, _ = build_trend_score_live_market_payload(
                    prepared,
                ...
                try:
                    ...
                    if lookup.conclusive and lookup.order is None:
                        _average_clear_intent(
                            state, note="average_post_failed_without_order",
                        )
                        raise RuntimeError(
                            f"Average order was not created: {post_exc}"
                        ) from post_exc
                    if not lookup.conclusive:
                        raise RuntimeError(
                            "Average POST outcome is inconclusive; the durable "
                            "intent is locked for automatic reconciliation"
                        ) from post_exc
                    order, response = lookup.order, {
                        "success": True, "recovered": True,
                ...
                if not order:
                    ...
                    if lookup.conclusive and lookup.order is None:
                        _average_clear_intent(
                            state, note="average_post_rejected_without_fill",
                        )
                        raise RuntimeError(
                            "Average order was rejected: "
                            + str(response.get("error") or response)[:300]
                        )
                    order = lookup.order
                if order:
                ...
                if not conclusive or filled is None:
                    raise RuntimeError(
                        "Average order status is inconclusive; the durable "
                        "intent is locked for automatic reconciliation"
                    )
                if filled == 0:
                    _average_clear_intent(
                        state, note="average_terminal_without_fill",
                    )
                    raise RuntimeError("Average order completed without a fill")
                if filled != int(preview["add_lots"]):
                    _average_rollback_partial(
                        state,
                        filled_lots=filled,
                        credentials=credentials,
                    )
                    raise RuntimeError(
                        f"Average filled only {filled}/{preview['add_lots']} "
                        "lots; the add-on was fully rolled back"
                    )
                live = _strict_realtime_position(
                    int(preview["product_id"]), credentials=credentials,
                ...
                if int(float(live.get("size") or 0)) != expected_signed:
                    raise RuntimeError(
                        "Delta aggregate quantity does not match the completed average"
                    )
                latest = _trend_score_auto_strict_json(state_path, {})
                intent = dict(latest.get("pending_average_intent") or {})
                if intent.get("client_order_id") != client_id:
                    raise RuntimeError("Average intent changed during reconciliation")
                intent.update({
                    "status": "FILLED_PENDING_PROTECTION",
                    "confirmed_filled_lots": filled,
                    "confirmed_order_id": (order or {}).get("id"),
                ...
                latest["pending_average_intent"] = intent
                _atomic_write_json(state_path, latest)
            ...
            while time.monotonic() < deadline:
                final_state = _trend_score_auto_strict_json(state_path, {})
                if (
                    int(final_state.get("average_count") or 0) >= 1
                    and not final_state.get("pending_average_intent")
                ):
                    health = _tp_monitor_payload().get("trend") or {}
                    protected = bool(
                        health.get("protection_established")
                    ...
                    if protected:
                        break
                time.sleep(0.25)
            if int(final_state.get("average_count") or 0) < 1:
                raise RuntimeError(
                    "Average fill is confirmed and protection reconciliation "
                    "is still in progress"
                )
            _trend_audit("trend_average_execute", {
                "mode": "live",
                "product_id": preview["product_id"],
                "previous_lots": preview["current_lots"],
                "added_lots": preview["add_lots"],
            ...
            return jsonify({
                ...
                "protection_established": protected,
                "message": (
                    "Average added and composite protection verified"
                    if protected else
